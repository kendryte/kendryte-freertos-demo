...
struct timeval get_time[CIPHER_MAX][AES_MODE_MAX][TEST_TIME_MAX];
                            ...
                            size_t data_size)
{
    ...
    else if (cipher_mod == AES_ECB)
    {
        ...
        aes_ecb192_hard_encrypt(key_addr, aes_data, data_size, aes_hard_out_data);
        if(get_time_flag)
    ...
    if (cipher_mod == AES_CBC)
        aes_cbc192_hard_decrypt(&cbc_context, aes_hard_out_data, data_size, aes_soft_out_data);
    else if (cipher_mod == AES_ECB)
        aes_ecb192_hard_decrypt(key_addr, aes_hard_out_data, data_size, aes_soft_out_data);
    else
        aes_gcm192_hard_decrypt(&gcm_context, aes_hard_out_data, data_size, aes_soft_out_data, gcm_hard_tag);
    total_check_tag = 0;
    for (i = 0; i < data_size; i++)
    {
