...
struct timeval get_time[CIPHER_MAX][AES_TEST_MAX][TEST_TIME_MAX];
    ...
    size_t data_size)
{
    ...
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_ECB][AES_SOFT][TEST_START], NULL);
        i = 0;
        if (data_size >= 16)
        {
        ...
        if(get_time_flag)
            gettimeofday(&get_time[AES_ECB][AES_SOFT][TEST_STOP], NULL);
    }
    else if (cipher_mod == AES_CBC)
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_CBC][AES_SOFT][TEST_START], NULL);
        AES_CBC_encrypt_buffer(aes_soft_out_data, aes_soft_in_data, data_size, input_key, iv);
        if(get_time_flag)
            gettimeofday(&get_time[AES_CBC][AES_SOFT][TEST_STOP], NULL);
    }
    else if (cipher_mod == AES_GCM)
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_GCM][AES_SOFT][TEST_START], NULL);
        ...
        if(get_time_flag)
            gettimeofday(&get_time[AES_GCM][AES_SOFT][TEST_STOP], NULL);
    }
    else
    {
        printf("\n error cipher \n");
    ...
    size_t data_size)
{
    ...
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_CBC][AES_HARD][TEST_START], NULL);
        aes_cbc192_hard_encrypt(&cbc_context, aes_data, data_size, aes_hard_out_data);
        if(get_time_flag)
            gettimeofday(&get_time[AES_CBC][AES_HARD][TEST_STOP], NULL);
    }
    else if (cipher_mod == AES_ECB)
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_ECB][AES_HARD][TEST_START], NULL);
        aes_ecb192_hard_encrypt(input_key, aes_data, data_size, aes_hard_out_data);
        if(get_time_flag)
            gettimeofday(&get_time[AES_ECB][AES_HARD][TEST_STOP], NULL);
    }
    else
    {
        if(get_time_flag)
            gettimeofday(&get_time[AES_GCM][AES_HARD][TEST_START], NULL);
        aes_gcm192_hard_encrypt(&gcm_context, aes_data, data_size, aes_hard_out_data, gcm_hard_tag);
        if(get_time_flag)
            gettimeofday(&get_time[AES_GCM][AES_HARD][TEST_STOP], NULL);
    }
}
    ...
    size_t data_size)
{
    ...
    if (cipher_mod == AES_CBC)
        aes_cbc192_hard_decrypt(&cbc_context, aes_hard_out_data, padding_len, aes_soft_out_data);
    else if (cipher_mod == AES_ECB)
        aes_ecb192_hard_decrypt(input_key, aes_hard_out_data, padding_len, aes_soft_out_data);
    else
        aes_gcm192_hard_decrypt(&gcm_context, aes_hard_out_data, padding_len, aes_soft_out_data, gcm_hard_tag);
    check_tag = 0;
...
int main(void)
{
    ...
    for (cipher = AES_ECB; cipher < AES_CIPHER_MAX; cipher++)
    {
        ...
        printf("[%s] [%ld bytes] [hard time = %ld us] [soft time = %ld us]\n", cipher_name[cipher],
                AES_TEST_DATA_LEN,
                (get_time[cipher][AES_HARD][TEST_STOP].tv_sec - get_time[cipher][AES_HARD][TEST_START].tv_sec) * 1000 * 1000 +
                (get_time[cipher][AES_HARD][TEST_STOP].tv_usec - get_time[cipher][AES_HARD][TEST_START].tv_usec),
                (get_time[cipher][AES_SOFT][TEST_STOP].tv_sec - get_time[cipher][AES_SOFT][TEST_START].tv_sec) * 1000 * 1000 +
                (get_time[cipher][AES_SOFT][TEST_STOP].tv_usec - get_time[cipher][AES_SOFT][TEST_START].tv_usec));
    }
    printf("aes-192 test pass\n");
    while (1)
        ;
    return 0;
}
