...
struct timeval get_time[TEST_TIME_MAX];
...
int main(void)
{
    ...
    gettimeofday(&get_time[TEST_START], NULL);
    sha256_hard_calculate(data_buf, sizeof(data_buf), hash);
    gettimeofday(&get_time[TEST_STOP], NULL);
    ...
    printf("[hard sha256][%ld bytes][time = %ld us]\n",
            sizeof(data_buf),
            (get_time[TEST_STOP].tv_sec - get_time[TEST_START].tv_sec) * 1000 * 1000 +
            (get_time[TEST_STOP].tv_usec - get_time[TEST_START].tv_usec));
