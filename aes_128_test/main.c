    ...
    size_t data_size)
{
    ...
    else if (cipher_mod == AES_ECB)
    {
        ...
        aes_ecb128_hard_encrypt(input_key, aes_data, data_size, aes_hard_out_data);
        if(get_time_flag)
    ...
    size_t data_size)
{
    ...
    if (cipher_mod == AES_CBC)
        aes_cbc128_hard_decrypt(&cbc_context, aes_hard_out_data, padding_len, aes_soft_out_data);
    else if (cipher_mod == AES_ECB)
        aes_ecb128_hard_decrypt(input_key, aes_hard_out_data, padding_len, aes_soft_out_data);
    else
        aes_gcm128_hard_decrypt(&gcm_context, aes_hard_out_data, padding_len, aes_soft_out_data, gcm_hard_tag);
    check_tag = 0;
